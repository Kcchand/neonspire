...
- provider_auto_create(vendor)  # optional; currently Milkyway
- result_ok(res) / result_error_text(res)
- all_providers, by_key, detect_by_name  # for backward compatibility
"""
...
# ---------------------------------------------------------------------------
try:
    from automation.ultrapanda_ui_bot import (  # absolute path
...
# ---------------------------------------------------------------------------
# Gameroom (your Playwright bot)
#
# Your actual file is automation/gameroom_bot.py (not automation/gameroom.py),
# so we import that. The file you pasted earlier only has async funcs + CLI,
# so we wrap them here into sync functions that return {"ok": True} like
# the rest of the providers.
# ---------------------------------------------------------------------------
try:
    from automation import gameroom_bot as grm  # <-- use the real filename
...
def result_ok(vendor: Optional[str], res: Any) -> bool:
    ...
    if isinstance(res, str) and any(
        k in res.lower() for k in ("ok", "success", "done", "loaded", "credited", "redeemed")
    ):
        return True
    return False
...
by_key: dict[str, Provider] = {
    ...
    "milkyway": Provider(
        key="milkyway",
        credit=milkyway.credit,
        redeem=milkyway.redeem,
        auto_create=milkyway.auto_create,
    ),
    "vblink": Provider(
        key="vblink",
...
if _GAMEROOM_IMPORTED:
    # we need to expose sync-ish functions; your gameroom_bot only has async
    # so wrap them here
...
# e.g. ['juwa','gv','milkyway','vblink','ultrapanda','yolo','gameroom']
...
def detect_vendor(game) -> Optional[str]:
    ...
    if not game:
        return None
    # prefer explicit vendor field if present
...
def provider_credit(vendor: str, account: str, amount: int, note: str = "") -> Any:
    ...
    return p.credit(account, amount, note)
...
def provider_redeem(vendor: str, account: str, amount: int, note: str = "") -> Any:
    ...
    if not p:
        return {"ok": False, "error": f"Unsupported vendor '{vendor}'"}
    return p.redeem(account, amount, note)
