...
class MilkywayUIBot:
    ...
    def create_player(self, account: str, password: str, nickname: Optional[str] = None) -> dict:
        """
        Create a Milkyway player and VERIFY it exists in the grid.
        If the user is not found after creation, raise MWError instead of faking success.
        """
        self.goto_user_management()
        self._click_create_player()
        self._fill_create_inline_or_modal_and_submit(account, nickname or account, password)

        # give the site a moment to update
        time.sleep(0.6)

        # verify: search the user and check that a row shows up
        try:
            self.search_user(account)
            bal = self._get_row_balance(account)   # None if no row
        except Exception:
            bal = None

        if bal is None:
            # dump debug info for you in .data/milkyway
            self._save_debug(self.page, f"Create player failed or user not found: {account}", prefix="create_fail")
            raise MWError(f"Create player might have failed for '{account}' (not found in grid).")

        # success: return normal payload
        return {
            "created": True,
            "account": account,
    ...
    def redeem(self, account_or_id: Union[str, int], amount: Union[int, float], note: str = ""):
        """
        Redeem from a player. If UI detection fails (e.g. 'Redeem dialog not found')
        but the balance actually decreased, treat it as SUCCESS instead of raising.
        """
        # Baseline balance before we touch anything
        self.search_user(account_or_id)
        before = self._get_row_balance(account_or_id)

        try:
            # Normal flow: open update / amount dialog and submit
            self._click_update_for_row(account_or_id)
            time.sleep(0.35)
            self._fill_amount_dialog("redeem", amount, note)

            # Verify by checking balance difference
            for verify_try in range(2):
                for _ in range(4):
                    time.sleep(0.7)
                    self.search_user(account_or_id)
                    ...
                    if abs((before - after) - float(amount)) < 1e-6:
                        # looks correct: before - after == amount
                        return
                if verify_try == 0:
                    # try the dialog flow once more if first verification failed
                    self._click_update_for_row(account_or_id)
                    time.sleep(0.2)
                    self._fill_amount_dialog("redeem", amount, note)

            # If we get here and had a valid "before", then balance didn’t change
            if before is not None:
                raise MWError("Redeem completed without balance change; likely failed silently.")

            # If we never had a valid baseline, just return and trust UI
            return

        except MWError as e:
            # SOFT FALLBACK:
            # Sometimes we get things like "Redeem dialog not found" even though
            # the site actually processed the redeem. Re-check the balance.
            try:
                self.search_user(account_or_id)
                after = self._get_row_balance(account_or_id)
            except Exception:
            ...
            if before is not None and after is not None and amt is not None:
                # If the balance went down by at least the redeem amount, treat as success
                if (before - after) >= amt - 1e-6:
                    self._save_debug(self.page, f"Redeem likely succeeded despite error: {e}", prefix="redeem_soft_ok")
                    return

            # Real failure: nothing changed, re-raise the error
            raise
...
def mw_create_player_auto():
    with mw_from_env().session() as bot:
        ...
        try:
            ...
            # enrich info so the Flask provider can use it easily
            info["password"] = password
            info["ok"] = True
            print(f"✅ Auto-created user '{account}' (nickname '{nickname}')")
            return info
        except MWError as e:
            print(f"❌ Milkyway create failed for '{account}': {e}")
            return {"ok": False, "error": str(e)}
        except Exception as e:
            print(f"❌ Milkyway create failed (unexpected error) for '{account}': {e}")
