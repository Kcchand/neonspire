# automation/firekirin_ui_bot.py
...
class FKError(RuntimeError):
    pass


@dataclass
class FKConfig:
    base: str = (os.getenv("FIREKIRIN_BASE_URL", "https://firekirin.xyz:8888") or "").rstrip("/")
    username: str = os.getenv("FK_USERNAME", "") or os.getenv("FIREKIRIN_USERNAME", "")
    password: str = os.getenv("FK_PASSWORD", "") or os.getenv("FIREKIRIN_PASSWORD", "")

    headless: bool = (os.getenv("FK_HEADLESS", "1") == "1")
    slowmo_ms: int = int(os.getenv("FK_SLOWMO_MS", "0"))
    timeout_sec: int = int(os.getenv("FK_TIMEOUT_SEC", "60"))
    force_login: bool = (os.getenv("FK_FORCE_LOGIN", "0") == "1")
    persist_dir: str = os.getenv("FK_PERSIST_DIR", ".data/firekirin")
    ...
    username_suffix: str = os.getenv("USERNAME_SUFFIX", "_fk")
    username_alphanum: bool = (os.getenv("USERNAME_ALPHANUM", "0") == "1")

    # UI nav label (left menu)
    user_nav_label: str = os.getenv("FK_USER_NAV_LABEL", "User Management") or "User Management"

    # Manual captcha fallback (if 2Captcha not available)
    manual_captcha_wait_sec: int = int(os.getenv("FK_MANUAL_CAPTCHA_WAIT_SEC", "180"))
...
class FireKirinUIBot:
    def __init__(self, cfg: Optional[FKConfig] = None):
        self.cfg = cfg or FKConfig()
        if not self.cfg.base:
            raise FKError("FIREKIRIN_BASE_URL missing.")
        if not self.cfg.username or not self.cfg.password:
            raise FKError("FK_USERNAME/FK_PASSWORD missing.")
    ...
    def _state_path(self) -> str:
        return str(Path(self.cfg.persist_dir) / "fk_state.json")
    ...
    def _goto(self, url: str, label: str = ""):
        ...
        for _ in range(3):
            try:
                ...
                last_err = e
                time.sleep(0.8)
        raise FKError(f"Failed to navigate to {label or url}: {last_err}")

    def _dismiss_any_ok(self):
        p = self.page
        assert p is not None
        for name in ("OK", "Ok", "ok", "Confirm", "CONFIRM"):
            try:
    ...
    # ──────────────────────────────────────────────────────────────────────
    # captcha (2Captcha base64) with manual fallback
    def _grab_login_captcha_bytes(self) -> bytes:
        """
        FireKirin login is same style: username/password + code input + captcha img near it.
    ...
    def _solve_captcha(self, img_bytes: bytes) -> str:
        if self.cfg.captcha_provider != "2captcha":
            raise FKError("Only 2Captcha is supported.")
        if not self.cfg.captcha_key:
            raise FKError("CAPTCHA_API_KEY missing.")
        ...
        if up.get("status") != 1:
            raise FKError(f"2Captcha in.php error: {up}")
        ...
        while True:
            ...
            if res.get("status") == 1:
                ...
                # keep raw (some captchas have letters); if you want digits-only, change here
                return code

            if res.get("request") != "CAPCHA_NOT_READY":
                raise FKError(f"2Captcha res.php error: {res}")

            if time.time() - t0 > self.cfg.captcha_timeout_sec:
                raise FKError("2Captcha timeout.")

    # ──────────────────────────────────────────────────────────────────────
    # frames (Store.aspx family)
    ...
    # ──────────────────────────────────────────────────────────────────────
    # login + ensure logged in (with 2captcha + manual fallback)
    def ensure_logged_in(self):
        ...
        # Manual captcha fallback (if no 2Captcha key or 2Captcha failed)
        if self.cfg.headless:
            raise FKError("Headless mode can't do manual captcha. Set FK_HEADLESS=0 then run again.")
        ...
        raise FKError("Timed out waiting for manual login.")
        ...
        while time.time() < deadline:
            for fr in p.frames:
                try:
                    ...
                    continue
            time.sleep(0.2)
        raise FKError("Could not find Create Player dialog inputs (need >=4).")
    ...
    def create_player(self, account: str, password: str, nickname: Optional[str] = None) -> dict:
        ...
        for sel in [
            ...
            try:
                ...
                pass
        if not clicked:
            raise FKError("Create Player button not found in User Management page.")
        ...
        if inputs.count() < 4:
            raise FKError(f"Create Player dialog missing inputs. Found {inputs.count()}")
        ...
        for sel in [
            ...
            try:
                ...
                pass
        if not submitted:
            raise FKError("Create Player submit button not found in dialog.")
    ...
    def create_player_auto(self) -> dict:
        ...
        print(f"✅ Auto-created FireKirin user '{acct}'")
        return out

    # ──────────────────────────────────────────────────────────────────────
    # Recharge / Redeem functionality (FIXED VERSION)
    ...
    def _click_in_main(self, labels: List[str]) -> None:
        ...
        for t in labels:
            ...
            try:
                ...
                pass
        raise FKError(f"Button not found: {labels}")
    ...
    def _click_update_for_row(self, account_or_id: Union[str, int]) -> bool:
        """Find and click the Update link for a specific user"""
        mf = self._main_frame() or self.page
        assert mf is not None
        ...
        if original_key.endswith('_fk'):
            key_without_suffix = original_key[:-3]  # Remove "_fk"
        ...
        for key in search_keys:
            try:
                # Strategy 1: Find cell with username, then find Update link in same row
                ...
                if user_cell.count() > 0 and user_cell.is_visible():
                    ...
                    # Look for Update link - it's usually in the first column
                    update_link = row.locator("xpath=.//a[contains(text(),'Update')]").first
                    
                    # If no Update text found, try any link in the row
                    if update_link.count() == 0:
                        update_link = row.locator("xpath=.//a").first
                    
                    if update_link.count() > 0 and update_link.is_visible():
                        print(f"✅ Found Update link for user '{key}'")
                        update_link.click(timeout=2500)
                        time.sleep(0.5)
                        return True
            except Exception:
                continue
        ...
        try:
            for key in search_keys:
                upd = mf.locator(
                    f"xpath=//tr[.//td[contains(text(),'{key}')]]//a[contains(text(),'Update')]"
                ).first
                if upd.count() > 0 and upd.is_visible():
                    print(f"✅ Found Update link via XPath for '{key}'")
                    upd.click(timeout=2500)
                    time.sleep(0.25)
                    return True
        except Exception:
            pass
        
        # Strategy 3: If only one Update link exists, click it
        try:
            update_count = mf.locator("text=Update").count()
            if update_count == 1:
                mf.locator("text=Update").first.click(timeout=2500)
                print("✅ Clicked the only Update link on page")
                time.sleep(0.25)
                return True
        except Exception:
            pass
        
        # Debug information if all strategies fail
        print(f"❌ Could not find Update link for user '{original_key}'")
        print(f"🔍 Tried searching for: {search_keys}")
        print(f"🔍 Update links on page: {mf.locator('text=Update').count()}")
        ...
        for _ in range(retries):
            ...
            while time.time() - t0 < 8:
                ...
                if container:
                    return container
                time.sleep(0.15)
        raise FKError(f"{cap} dialog not found.")
    ...
    def _fill_amount_dialog(self, kind: str, amount: Union[int, float], note: str = ""):
        ...
        if not amount_input:
            raise FKError(f"{cap} amount input not found.")
        ...
        for sel in [
            ...
            raise FKError(f"{cap} submit button not found.")
    ...
    def recharge(self, account_or_id: Union[str, int], amount: Union[int, float], note: str = "") -> dict:
        ...
        # Step 2: Click Update link for the user
        if not self._click_update_for_row(account_or_id):
            raise FKError("Update link not found after search.")
...
def main():
    ...
    ap = argparse.ArgumentParser("FireKirin UI Bot")
    ...
    with bot.session() as b:
        ...
        if args.cmd == "login":
            print("✅ FireKirin logged in (2Captcha or manual captcha) + session saved")
            return
...
# ──────────────────────────────────────────────────────────────────────────────
# SYNC WRAPPERS FOR player_bp.py
# Add these functions at the END of the file, before if __name__ == "__main__":

def auto_create_sync() -> dict:
    """
    Auto-create a FireKirin account (for player_bp.py).
    ...
    try:
        ...
        return {"ok": False, "error": f"FireKirin auto_create failed: {str(e)}"}


def recharge_sync(account: str, amount: float, note: str = "") -> dict:
    """
    Recharge a FireKirin account (for player_bp.py).
    """
    try:
        bot = FireKirinUIBot(FKConfig())
        with bot.session() as b:
        ...
        return {"ok": False, "error": f"FireKirin recharge failed: {str(e)}"}


def redeem_sync(account: str, amount: float, note: str = "") -> dict:
    """
    Redeem from a FireKirin account (for player_bp.py).
    """
    try:
        bot = FireKirinUIBot(FKConfig())
        with bot.session() as b:
        ...
        return {"ok": False, "error": f"FireKirin redeem failed: {str(e)}"}
