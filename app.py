...
from flask_socketio import join_room, leave_room, rooms  # Add these
...
print("✅ TELEGRAM_BOT_TOKEN (first 15):", os.getenv("TELEGRAM_BOT_TOKEN")[:15] if os.getenv("TELEGRAM_BOT_TOKEN") else "Not set")

# Import keepalive and blueprints
...
# Chat blueprint - REQUIRED for Telegram sync
try:
    from chat_bp import chat_bp
    CHAT_BP_AVAILABLE = True
except ImportError as e:
    print(f"❌ chat_bp.py not found or has errors: {e}")
    CHAT_BP_AVAILABLE = False
    chat_bp = None
except Exception as e:
    print(f"⚠️  Error importing chat_bp: {e}")
    CHAT_BP_AVAILABLE = False
    chat_bp = None
...
# ------------------------------------------------------------------
# ------------------------------------------------------------------
# Socket.IO with Chat Integration
# ------------------------------------------------------------------
ASYNC_MODE = os.getenv("ASYNC_MODE", "threading").strip().lower()
socketio = SocketIO(async_mode=ASYNC_MODE, cors_allowed_origins="*", logger=True, engineio_logger=True)
...
def create_app():
    ...
    if chat_bp and CHAT_BP_AVAILABLE:
        app.register_blueprint(chat_bp)
        print("✅ Chat blueprint registered")
    else:
        print("❌ Chat blueprint NOT registered - Telegram sync will not work")
    ...
    def index():
        ...
        settings = db.session.get(PaymentSettings, 1)
        bonus_settings = db.session.get(BonusSettings, 1)
        if not bonus_settings:
            # Create default if missing
            bonus_settings = BonusSettings(id=1)
        ...
        if not current_user.is_authenticated:
            return render_template(
                ...
                game_login_ids=game_login_ids,
                # 🔹 Add bonus stats with default values
        ...
        # 🔹 Calculate active bonuses count
        active_bonuses_count = 0
        try:
    ...
    def handle_join_thread(data):
        ...
        if thread_id:
            ...
            emit("joined_thread", {"thread_id": thread_id}, room=request.sid)

    @socketio.on("new_message", namespace="/chat")
    def handle_new_message(data):
        """Forward messages between participants"""
        thread_id = data.get("thread_id")
        message_data = data.get("message")
        
        if thread_id and message_data:
            # Emit to everyone in the thread room (excluding sender)
            emit("new_message", 
                message_data, 
    ...
    with app.app_context():
        ...
        if not _has_col("users", "bonus_eligible"):
            _add_col(
                "ALTER TABLE users ADD COLUMN bonus_eligible BOOLEAN DEFAULT 0",
                "ALTER TABLE users ADD COLUMN IF NOT EXISTS bonus_eligible BOOLEAN DEFAULT FALSE",
                "ALTER TABLE users ADD COLUMN bonus_eligible TINYINT(1) DEFAULT 0",
            )
        ...
        # 🔹 bonus-related columns that are missing
        if not _has_col("users", "deposit_count"):
            _add_col("ALTER TABLE users ADD COLUMN deposit_count INTEGER DEFAULT 0")
        if not _has_col("users", "signup_bonus_claimed"):
            _add_col("ALTER TABLE users ADD COLUMN signup_bonus_claimed BOOLEAN DEFAULT 0")
        if not _has_col("users", "signup_bonus_amount"):
            _add_col("ALTER TABLE users ADD COLUMN signup_bonus_amount REAL DEFAULT 0.0")
        if not _has_col("users", "signup_bonus_claimed_at"):
            _add_col("ALTER TABLE users ADD COLUMN signup_bonus_claimed_at DATETIME")
        if not _has_col("users", "regular_bonus_last_claimed"):
            _add_col("ALTER TABLE users ADD COLUMN regular_bonus_last_claimed DATETIME")
        if not _has_col("users", "available_bonus"):
            _add_col("ALTER TABLE users ADD COLUMN available_bonus REAL DEFAULT 0.0")
...
def init_bonus_settings():
    ...
    try:
        ...
        if BonusSettings.query.count() == 0:
            default_settings = BonusSettings(
                id=1,
                signup_active=True,
                signup_percentage=100,
                signup_max_amount=100.0,
                signup_min_deposit=20.0,
                signup_wagering=30,
                regular_active=True,
                regular_percentage=50,
                regular_max_amount=50.0,
                regular_min_deposit=10.0,
                regular_wagering=25
            )
            db.session.add(default_settings)
            db.session.commit()
            print("✅ Bonus settings initialized")
        else:
            print("✅ Bonus settings already exist")
    except Exception as e:
        print(f"⚠️ Could not init bonus settings: {e}")
        db.session.rollback()
...
# Make socketio available globally for telegram_bot.py
